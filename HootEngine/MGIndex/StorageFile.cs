...
namespace Hoot
{
    ...
    internal class StorageFile<T>
    {
        ...
        private IBson _bson = JsonManager.GetBsonNet();
        ...
        //private string _recExtension = ".mgrec";
        private string _recExtension = Global.STExt;
        ...
        /// <param name="StorageOnlyMode">= true -> don't create mgrec files (used for backup and replication mode)，是否唯一，是否切割文件</param>
        ...
        private long internalWriteData(StorageItem<T> meta, byte[] data, bool raw)
        {
            lock (_readlock)
            {
                ...
                if (raw == false)
                {
                    ...
                    //byte[] metabytes = fastBinaryJSON.BJSON.ToBJSON(meta, new fastBinaryJSON.BJSONParameters { UseExtensions = false });//fastBinaryJSON
                    byte[] metabytes = _bson.SerializeObject(meta);

                    // write header info
                    _datawrite.Write(new byte[] { 1 }, 0, 1); // FEATURE : add json here, write bson for now
                    _datawrite.Write(Helper.GetBytes(metabytes.Length, false), 0, 4);//写入流的长度，4位byte
                    _datawrite.Write(metabytes, 0, metabytes.Length);
                    // update pointer
                    _lastWriteOffset += metabytes.Length + 5;//1位特性，4位长度
                }
                else
                {
                    // write header info
                    _datawrite.Write(new byte[] { 0 }, 0, 1); // write raw
                    _datawrite.Write(Helper.GetBytes(data.Length, false), 0, 4);//写入流的长度，4位byte
                    // update pointer
                    _lastWriteOffset += 5;
                }
        ...
        private StorageItem<T> ReadMetaData(FileStream fs, out int metasize)
        {
            ...
            if (type > 0)
            {
                ...
                StorageItem<T> meta;
                if (type == 1)
                    //meta = fastBinaryJSON.BJSON.ToObject<StorageItem<T>>(hdr);//fastBinaryJSON
                    meta = _bson.DeserializeObject<StorageItem<T>>(hdr); 
                else
                {
                    string str = Helper.GetString(hdr, 0, (short)hdr.Length);
                    //meta = fastJSON.JSON.ToObject<StorageItem<T>>(str);//fastJSON
                    meta =_json.DeserializeObject<StorageItem<T>>(str);
                }
                return meta;
            }
            else
