        ...
        internal void SavePage(Page<T> node)
        {
            lock (_fileLock)
            {
                ...
                if (_externalStrings)
                {
                    ...
                    //blocknum = _strings.SaveData(node.DiskPageNumber.ToString(), fastBinaryJSON.BJSON.ToBJSON(keys));//fastBinaryJSON
                    blocknum = _strings.SaveData(node.DiskPageNumber.ToString(), _bson.SerializeObject(keys));
                }
                // node children
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        else
                        {
                            if (keys == null)
                            {
                                ...
                                //keys = (object[])fastBinaryJSON.BJSON.ToObject(bb);//fastBinaryJSON
                                keys = _bson.DeserializeObject<object[]>(bb);
                            }
                            key = (T)keys[i];
                        }
